...
test("computeExecKpis: the real decisions_seed.json alone (no windowResults) — the seeded-alone path", () => {
  ...
  assert.equal(kpis.hasSeededData, true);
  // Regenerated by tools/seed_decisions.mjs, which runs every historical row through the REAL
  // evaluator (createWindowEvaluator(), fed by demand.js's actual 15-minute demand curve at each
  // row's real incident_date/incident_time) instead of the old seed_decisions.py's closed-form
  // single-slice-at-shoulder-multiplier approximation, which could never exceed C_closed and so
  // produced revenueAtRiskUsd.point === 0 for every row, every time (the "seed revenue $0" bug).
  // Some historical closures overlapped an AM/PM peak or weekend-midday window and now correctly
  // show non-trivial revenue-at-risk; others genuinely didn't and stay at $0 — see the
  // "revenue-at-risk is not uniformly zero" test below for that mix.
  assert.equal(kpis.revenueProtected, 9348.03);
  ...
  // every row's duration contributes. Sum of the 15 rows' window.durationHours (unchanged by the
  // evaluator swap — durations come straight from the historical rows, not the evaluator).
  assert.equal(kpis.closureHoursAvoided, 55.5);
  assert.equal(kpis.pctOptimalWindow, 100);
  assert.equal(kpis.secondaryIncidentsAvoided, 12.45);
});

test("decisions_seed.json: revenue-at-risk is not uniformly zero", () => {
  const seed = loadSeed();
  assert.ok(
    seed.some((r) => r.revenueAtRiskUsd.point > 0),
    "expected at least one seeded row to land in a peak/high-demand window with non-zero revenue-at-risk"
  );
  assert.ok(
    seed.some((r) => r.revenueAtRiskUsd.point === 0),
    "expected at least one seeded row to genuinely have zero revenue-at-risk (off-peak closure) — not every row should be non-zero either"
  );
});
...
test("renderExecKpiStrip: renders all four KPI values", () => {
  ...
  const expectedRevenue = kpis.revenueProtected.toLocaleString("en-US", {
    style: "currency",
    currency: "USD",
    maximumFractionDigits: 0,
  });
  assert.ok(el.innerHTML.includes(expectedRevenue)); // revenueProtected — no longer the $0 bug
  assert.ok(el.innerHTML.includes("55.5"));
  assert.ok(el.innerHTML.includes("100.0%"));
  assert.ok(el.innerHTML.includes(kpis.secondaryIncidentsAvoided.toFixed(2)));
});
